...
public class UserServiceImpl implements UserService {
	...
	public UserDto save(UserDto dto) {
		...
		if (userAlreadyExists(baseInfoDto.getEmail())) {
			 log.error("An user with the same email already exists {}", baseInfoDto.getEmail());
	...
	public boolean userAlreadyExists(String email) {
	    return userRepository.findByBaseInfoEmail(email).isPresent();
	}
	...
	public UserDto findByEmail(String email) {
		...
		return userRepository.findByBaseInfoEmail(email).map(UserDto::fromEntity).orElseThrow(
	...
	private void validate(ChangePasswordDto dto) {
		...
		if (!StringUtils.hasLength(dto.getPassword()) || !StringUtils.hasLength(dto.getConfirmedPassword())) {
	...
	public List<UserDto> findAllByCompanyId(Integer id) {
		...
		return userRepository.findAllByCompanyId(id).stream().map(UserDto::fromEntity).collect(Collectors.toList());
